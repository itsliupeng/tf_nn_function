            ...
            if is_training is not None else NoOpScope()):
                ...
                if include_root_block:
                    if output_stride is not None:
                        ...
                        output_stride /= 4
                    net = resnet_utils.conv2d_same(net, 64, 3, stride=2, scope='conv1_0')
                    net = slim.batch_norm(net, activation_fn=tf.nn.relu, scope='conv1_0_bn')
                    net = resnet_utils.conv2d_same(net, 64, 3, stride=1, scope='conv1_1')
                    net = slim.batch_norm(net, activation_fn=tf.nn.relu, scope='conv1_1_bn')
                    net = resnet_utils.conv2d_same(net, 64, 3, stride=1, scope='conv1_2')
                    net = slim.batch_norm(net, activation_fn=tf.nn.relu, scope='conv1_2_bn')
                ...
                feature_map = tf.identity(net, name='feature_map')
                ...
                if num_classes:
                    ...
                    heat_map_features = slim.conv2d(feature_map, num_classes, [1, 1], activation_fn=None,
                                                    normalizer_fn=None, scope='logits', reuse=True)
